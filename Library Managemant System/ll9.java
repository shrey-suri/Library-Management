//Library Management System RETURN Books
import java.awt.*;
...
JPanel p=new JPanel();
p.setLayout(null);
ImageIcon pic=new ImageIcon("return.jpg");
final JLabel l3=new JLabel(pic);
l3.setBounds(10,70,400,500);
p.add(l3);
JLabel l=new JLabel("RETURN BOOKS");
...
ResultSet rs=st.executeQuery("select * from Issued where Code='"+t1.getText()+"'");
while(rs.next())
{
...
ResultSet rst=s.executeQuery("select * from Issued where Code='"+t1.getText()+"'");
while(rst.next())
{
String re="insert into book values('"+rst.getString(1)+"','"+rst.getString(2)+"','"+rst.getString(3)+"','"+rst.getString(4)+"')";
s.executeUpdate(re);
}
String o="delete * from Issued where Code='"+t1.getText()+"'";
s.executeUpdate(o);

lla f2=new lla();
        f2.setSize(1366,728);
        f2.setVisible(true);
		f2.setTitle("BOOK RETURNED PAGE");
